import { useGetInfoNotificationsQuery, useMeQuery } from '@/app/api'
...
export const Notifications = () => {
  ...
  const { data: info } = useGetInfoNotificationsQuery(me?._id as string, {
    skip: !me?._id,
  });
      ...
      <div className='mt-24 w-[95%] m-auto p-[12px] flex flex-col gap-y-5 mb-20'>
        {info?.length === 0 ? (
          ...
          info?.map((item) => (
            ...
            >
              <h4 className=' text-white text-xl font-semibold mb-1 text-center'>
                {item.title}
              </h4>
